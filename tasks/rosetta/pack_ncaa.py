...
class PackNCAATask(RosettaTask):
    structural_metrics = [
        "cav_vol",
        "rg",
    ...
    def __init__(self, config: RosettaTaskConfig, workdir: Path = None):
        ...
        ncaas = self.config.ncaas

        if len(ncaas) != 1:
            raise ValueError(
                f"PackNCAATask supports exactly one NCAA type, but got: {ncaas}"
            )
        self.ncaa = ncaas[0]
    ...
    def build_history(
        ...
        task_prompt = self.config.prompt.format(
            ncaa_name=self.ncaa.name,
            ncaa_code3=self.ncaa.code3,
            ncaa_description=self.ncaa.description,
        ).strip()
        reasoning_formatting = f"{agent.model.config.reasoning_formatting}\n" or ""
    ...
    def summarize_ensemble(
        ...
        ncaa_inclusion_summary = summarize_ncaa_inclusion(
            designs=designs, ncaa_code3=self.ncaa.code3, q=self.q
        )
        ...
        return template.format(
            ...
            ncaa_inclusion_summary=ncaa_inclusion_summary,
